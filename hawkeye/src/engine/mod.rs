        ...
        impl GitRepo {
                ...
                S: AsRef<OsStr>,
            {
                let arguments = arguments
                    ...
                    .map(|argument| argument.as_ref().to_owned())
                    .collect::<Vec<_>>();
                // File-backed stdin and stderr let Git consume and produce both while stdout is
                // parsed synchronously, without another thread or the risk of a
                // full pipe blocking the child.
                let mut stderr = tempfile::tempfile().map_err(|err| {
                    Error::new(ErrorKind::Unexpected, "cannot create Git stderr buffer")
                        .with_source(err)
                })?;
                let stderr_writer = stderr.try_clone().map_err(|err| {
                    Error::new(ErrorKind::Unexpected, "cannot clone Git stderr buffer")
                        .with_source(err)
                })?;
        ...
        impl GitRepo {
            ...
            fn read_history(
                ...
                if pathspecs.is_some() {
                    ...
                    // `git log --stdin` is line-delimited. Fall back to an unfiltered traversal for
                    // the rare repository containing a selected path with an
                    // embedded line ending.
                    arguments.push("--");
                }
                // Empty NUL-delimited records cannot be file paths, so a pair unambiguously frames
                // commit metadata without relying on Git's quoting rules or
                // reserving a valid path byte.
                self.read_stdout(
        ...
        fn parse_history(
            ...
            loop {
                ...
                let path = if expecting_first_path {
                    expecting_first_path = false;
                    // `--name-only -z` inserts one newline between the pretty header and its first
                    // path.
